...
class DeepQNetwork:
    def __init__(self, actions, learning_rate=0.003, reward_decay=0.9, random_probability=1.0, mem_size=3000,
        ...
        # self.predict_input_state = tf.placeholder(shape=(None, 1), dtype=tf.float32, name="predict_input_state")
        # self.target_input_state = tf.placeholder(shape=(None, 1), dtype=tf.float32, name="target_input_state")
        # ================= State action input ========================
    ...
    def choose_action(self, current_state):
        ...
        if np.random.uniform() <= self.random_probability:
            ...
            # [action] = self.sess.run(idx_most, feed_dict={self.predict_input_state: [[current_state]]})
            ...
            if self.mem[index][Experience.DONE]:
                ...
                _, [[loss]] = self.sess.run([self.train_op, self.loss],
                                            feed_dict={
                                                ...
                                                self.y: [[self.mem[index][Experience.REWARD]]]
                                            }
                                            )
            else:
                for i in range(self.actions):
                ...
                y = self.mem[index][Experience.REWARD] + self.gamma * target_max_q_value

                _, [[loss]] = self.sess.run((self.train_op, self.loss),
                                            feed_dict={
                                                self.predict_input_source: [[self.mem[index][Experience.CURRENT_STATE],
                                                                             self.mem[index][Experience.ACTION]]],
                                                # self.predict_input_source: [[1, 2]],
                                                self.y: [[y]],
                                            }
                                            )
